...
import { STUDIO_SOCIAL, StudioSocialItem } from "@/lib/data";
import { Badge } from "@/components/ui/Badge";

export default function SocialMediaShowcase() {
  const [selectedSocial, setSelectedSocial] = useState<StudioSocialItem | null>(null);

  return (
    <section className="py-24 bg-[#0B0B0E] border-t border-white/10 relative z-10 overflow-hidden">
      {/* Background Lighting Glow */}
      <div className="absolute top-1/2 left-1/3 w-[600px] h-[600px] bg-[#E6C594]/5 rounded-full blur-[170px] pointer-events-none" />
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div className="space-y-4 max-w-2xl">
            <Badge variant="gold" className="gap-1.5">
              <Share2 className="w-3.5 h-3.5 text-[#E6C594]" />
              ATELIER SOCIAL MEDIA COLLECTION
            </Badge>
            <h2 className="text-3xl sm:text-5xl font-serif-editorial font-medium text-[#F9F8F6] leading-tight">
              Selected Social <span className="italic gold-gradient-text">Media Graphics.</span>
            </h2>
            <p className="text-base text-[#8C8C9A] font-light leading-relaxed">
              Multi-slide Instagram carousels, promotional product launch posts, story frame suites, and campaign banners created by Nuvora Studio.
            </p>
          </div>
          ...
          <div className="text-xs font-mono text-[#8C8C9A] bg-white/5 px-4 py-2 rounded-full border border-white/10 self-start md:self-end">
            10 FEATURED SOCIAL CREATIONS
          </div>
        </div>

        {/* 10 Social Media Showcase Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {STUDIO_SOCIAL.map((social, index) => (
            <motion.div
              key={social.id}
          ...
          >
            ...
            >
              ...
              <button
                ...
                aria-label="Close social lightbox"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-1 pr-12">
                <span className="text-xs font-mono text-[#E6C594] uppercase tracking-widest">
                  {selectedSocial.category} • {selectedSocial.year}
                </span>
              ...
              <div className="flex items-center justify-between pt-2 text-xs font-mono text-[#8C8C9A]">
                <span>Nuvora Studio Social Media Creation</span>
                <span className="text-[#E6C594]">Instagram Carousel &amp; Story System</span>
              </div>
            </motion.div>
          </div>
        )}
